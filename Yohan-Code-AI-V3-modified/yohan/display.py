...
def print_help():
    ...
    commands = [
        ...
        ("/keys check <p> <n>",       "Check if key N for provider P works"),
        ("/keys select <p> <n>",       "Manually select key N for provider P"),
        ("/keys list [provider]",     "List all keys for a provider"),
        ("/keys reset [provider]",    "Reset key rotation to auto"),
...
def print_key_check_result(provider_id: str, key_num: int, result: dict):
    """Display result of /keys check command."""
    console.print()
    if result.get("ok"):
        console.print(
            Text(f"  ✓ Key {key_num} for {provider_id}: {result.get('status', 'OK')}", style="yohan.success")
        )
    else:
        console.print(
            Text(f"  ✗ Key {key_num} for {provider_id}: {result.get('error', 'Unknown error')}", style="yohan.error")
        )
    console.print()


def print_key_selected(provider_id: str, key_num: int):
    """Display result of /keys select command."""
    console.print()
    console.print(
        Text(f"  ✓ Key {key_num} selected for {provider_id}", style="yohan.success")
    )
    console.print(
        Text(f"    Auto-rotation disabled. Using key {key_num} exclusively.", style="dim")
    )
    console.print()


def print_keys_list(provider_id: str, keys_info: list[dict]):
    """Display detailed list of keys for a provider."""
    from yohan.config import PROVIDERS
    console.print()
    console.print(Text(f"  Keys for {PROVIDERS[provider_id]['name']}:", style="yohan.primary"))
    console.print()
    ...
    for info in keys_info:
        ...
        console.print(Text(f"{prefix} Key {idx}: {masked} {status}", style=style))
