...
def make_agent(chat_id: str, provider_id: str, model: str | None) -> Agent:
    ...
    def on_tool_call(name: str, params: dict):
        from yohan.sandbox import get_sandbox_files
        print()  # newline after streaming

        if name == "write_file":
            path = params.get("path", "")
            verb = ("Updating" if (get_sandbox_files(chat_id) / path).exists()
                    else "Creating")
            display.print_action(f"{verb} file", path)
        elif name == "bash":
            command = params.get("command", "").strip()
            first_line = command.splitlines()[0] if command else ""
            if len(first_line) > 80:
                first_line = first_line[:77] + "..."
            display.print_action("Running command", first_line)
        elif name == "read_file":
            display.print_action("Reading file", params.get("path", ""))
        elif name == "list_files":
            display.print_action("Listing files", params.get("path", "."))
        elif name == "search_files":
            display.print_action("Searching files", repr(params.get("pattern", "")))
        else:
            display.print_action(name, str(params)[:60])

    def on_tool_result(name: str, result: dict):
        from yohan.tools import format_tool_result
        if "error" in result:
            display.print_error(f"{name}: {result['error']}")
            return
        if name == "write_file":
            # Compact confirmation with file size — no noisy panel
            import os as _os
            path = result.get("path", "")
            try:
                size = _os.path.getsize(path)
                human = f"{size / 1024:.1f} KB" if size >= 1024 else f"{size} B"
                display.print_success(f"Saved {path} ({human})")
            except OSError:
                display.print_success(f"Saved {path}")
            return
...
def run_repl(provider_id: str, model: str | None, chat_id: str | None = None):
    ...
    while True:
        ...
        # Tolerate typos like "//exit" — collapse duplicate leading slashes
...
Examples:
  ...
  yohan --list-models groq       Show models for a provider
    ...
    parser.add_argument(
        "--provider", "-p",
        help="AI provider to use (groq, gemini, cloudflare, openrouter, orcarouter, openai, anthropic, mistral, cohere)",
    )
    parser.add_argument(
        "--model", "-m",
