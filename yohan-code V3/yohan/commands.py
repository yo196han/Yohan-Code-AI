...
def handle_command(cmd_line: str, agent) -> CommandResult:
    ...
    # ── Keys status ───────────────────────────────────────────────────────────
        ...
        # Suggest the closest known command for typos like /exeit, /qit
        import difflib
        known = ["/help", "/clear", "/reset", "/provider", "/model", "/models",
                 "/providers", "/skill", "/sandbox", "/sandboxes", "/history",
                 "/keys", "/exit", "/quit", "/q"]
        match = difflib.get_close_matches(cmd, known, n=1, cutoff=0.6)
        hint = f"  Did you mean {match[0]}?" if match else "  (type /help for commands)"
        display.print_error(f"Unknown command: {cmd}{hint}")
